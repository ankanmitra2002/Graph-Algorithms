...
void addEdge(vector<int> adjList[], int v1, int v2) {
    ...
    // Vertex v1 is added to the adjacency list of v2 (undirected graph).
...
// A function is defined for performing BFS traversal and finding the shortest path from the source to the destination.
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < adjList[curr].size(); i++) {
            ...
            if (!visited[neighbour]) {
                ...
                distance++;  // The distance is incremented.

                // If the destination node is reached, the distance is returned.
...
int main() {
    ...
    // The shortest path is found using BFS.
    int distance = bfs(source - 1, destination - 1, visited, adjList, parent);
    if (distance == -1) {
        cout << "There are no path between " << source << " and " << destination << endl;
    } else {
        cout << "The shortest distance between " << source << " and " << destination << ": " << distance << endl;
        cout << "The corresponding path is:" << endl;
        bfsTreePathDisplay(source - 1, destination - 1, parent);  // The shortest path is displayed.
    }
